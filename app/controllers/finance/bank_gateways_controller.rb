class Finance::BankGatewaysController < Finance::BaseController
  ...
  def import
    ...
    location = @bank_gateway.connector.pay_and_import_url callback_uri, current_user, nil, reconfigure: reconfigure
  ...
  def pay
    ...
    location = @bank_gateway.connector.pay_and_import_url callback_uri, current_user, { sepaCreditTransferPayments: payments }
    redirect_to location, status: :found
  end
